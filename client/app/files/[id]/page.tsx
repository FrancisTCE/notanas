...
export default function FileDetailsPage({ params }: { params: { id: string } }) {
  ...
  const openFile = async (id: string) => {
    ...
    try {
      const response = await fetch(`${serverURL}/api/v1/files/children?id=${id}`, {
        method: "GET",
        headers: {
  ...
  useEffect(() => {
    ...
    if (!initialized) {
      openFile(""); // Replace "" with "root" or any default value for initial load
      setInitialized(true); // Set initialized to true so this doesn't run again
    } else if (id) {
      openFile(id); // Fetch based on the current id
    }
  }, [router, loading, initialized, id]);
